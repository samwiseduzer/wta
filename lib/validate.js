...
module.exports = function(filepath,missingErr,options,cb){
    fs.exists(filepath, function(exists){
        ...
        fs.readFile(filepath, 'utf8', function(err,str){
            ...
            if(options.isJasmine && data.globals.includes('beforeEach')){
                data.globals.splice(data.globals.indexOf('beforeEach'),1);
            }
            if(options.isJasmine && data.globals.includes('afterEach')){
                data.globals.splice(data.globals.indexOf('afterEach'),1);
            }
            if(options.isJasmine && data.implieds){
                for(let i = data.implieds.length - 1; i >= 0; i--){
                    if(['describe','it','expect','beforeEach','afterEach'].concat(options.fnName).includes(data.implieds[i].name)) data.implieds.splice(i,1);
                }
            }
            data.unused = data.unused ? data.unused.filter(function(element){
                return element.name !== (options.isJasmine ? options.fnName[0] : options.fnName);
            }) : [];
            ...
            if(!options.isJasmine && !data.functions){
                ...
                data.functions.forEach(function(fn){
                    if(options.fnName.includes(fn.name)){
                        Util.fatalError(`The function "${fn.name}" should not be defined within your tests - this prevents us from testing our versions of the function "${fn.name}"`);
                    }
                });
            }
            else{
