...
Problem in "${error.hasOwnProperty('evidence') ? error.evidence.trim() : '<NO EVIDENCE>'}" at line ${error.line}, character ${error.character}`));errCount++;});
                console.log('');
                cb(errors,null);
            }
            if(data.unused && data.unused.length){
                data.unused.forEach((warning) => {console.log(chalk.yellow(`(${warnCount}) Unused variable on line ${warning.line}: ${warning.name}`));warnCount++;});
            }
            if(data.implieds && data.implieds.length){
                data.implieds.forEach((warning) => {console.log(chalk.yellow(`(${warnCount}) Undeclared variable on line ${warning.line}: ${warning.name} must be declared with a keyword like var, let, or const`));warnCount++;});
            }
            if(data.globals && data.globals.length){
                data.globals.forEach((warning) => {console.log(chalk.yellow(`(${warnCount}) Global variable on line ${warning.line}: ${warning.name} is a global variable - try to solve this problem within just the scope of your function`));warnCount++;});
            }
